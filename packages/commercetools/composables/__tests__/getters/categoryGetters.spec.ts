...
describe('[commercetools getters] category Getters', () => {
  ...
  it('returns all products', () => {
    expect(getProducts(category)).toEqual(category._products);
  });

  it('returns master products', () => {
    expect(getProducts(category, { master: true })).toEqual([
      { _name: 'prod1',
        _master: true },
      { _name: 'prod5',
        _master: true }
  ...
  it('returns null when there is no category', () => {
    expect(getTree(null)).toBe(null);
  });

  it('returns category tree', () => {
    const categoryRoot = getTree(category);
